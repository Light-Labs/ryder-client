...
export default class RyderSerial extends Events.EventEmitter {
  ...
  private serial_data(data: Uint8Array): void {
    this.log(LogLevel.DEBUG, 'data from Ryder', { data: "0x" + Buffer.from(data).toString("hex") });
    ...
    if (this[state_symbol] === State.IDLE) {
      ...
      if (this[state_symbol] === State.SENDING) {
        ...
        if (data[0] === RESPONSE_OK || data[0] === RESPONSE_SEND_INPUT || data[0] === RESPONSE_REJECTED) {
          this.log(LogLevel.DEBUG, "---> (while sending): RESPONSE_OK or RESPONSE_SEND_INPUT or RESPONSE_REJECTED");
        ...
        for (let i = offset; i < data.byteLength; ++i) {
          ...
          if (!this[train_symbol][0][3]) { // previous was not escape byte
            if (b === RESPONSE_ESC_SEQUENCE) {
              ...
              this.log(LogLevel.DEBUG, "---> READING SUCCESS resolving output buffer", { output_buffer: "0x" + Buffer.from(this[train_symbol][0][4]).toString("hex") });
  ...
  public send(data: string | number, prepend?: boolean): Promise<string> {
    ...
    this.log(LogLevel.DEBUG, 'queue data for Ryder: ' + data.length + ' byte(s)', { bytes: Buffer.from(data).toString("hex") });
